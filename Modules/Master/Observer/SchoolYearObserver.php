...
class SchoolYearObserver
{
    public function creating(SchoolYear $model)
    {
        ...
        Activity::record("Tambah Periode", ['original' => $model]);
    }
    ...
    public function updating(SchoolYear $model)
    {
        ...
        Activity::record("Ubah Periode", Activity::parse($model->getDirty(), $model->getOriginal()));
    }

    public function deleting(SchoolYear $model)
    {
        Activity::record("Hapus Periode", ['original' => $model->fill(['deleted_by' => Auth::id()])]);
