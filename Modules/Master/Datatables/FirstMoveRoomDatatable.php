...
class FirstMoveRoomDatatable extends TableComponent
{
    ...
    public function down(string $fromRoom, array $studentIds): Event
    {
        if (is_null($this->room)) {
            return $this->warning('Oops!', 'Pilih Wilayah tujuan terlebih dahulu.');
        }

        if ($fromRoom === $this->room) {
            return $this->warning('Oops!', 'Wilayah sebelumnya tidak boleh sama dengan Wilayah tujuan.');
        }

        $students = Student::whereIn('id', $studentIds)->whereNotIn('room_id', [$this->room])->get();

        if (!$students) {
            return $this->error('Oops!', 'Klien tidak ditemukan.');
        }
        ...
        return $this->success('Berhasil!', 'Wilayah Klien berhasil diubah.');
    }
